...
class `1_Sciezka` : Fragment() {

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val walkRecycler = inflater.inflate(R.layout.s1_sciezka_xml, container, false) as RecyclerView
        val walkNames = Sciezka.info_easy.map { it.name }.toTypedArray()
        val walkImages = Sciezka.info_easy.map { it.imageId }.toIntArray()
        ...
        adapter.setListener(object : Adapt_Zdjecia.Listener {
            override fun onClick(position: Int) {
                val intent = Intent(activity, Info_Sciezka::class.java).apply {
                    ...
                    putExtra(Info_Sciezka.EXTRA_WALK_CATEGORY, "Łatwe ścieżki")
                }
